...
export const rolesDb = {
  findByAuth(auth: string) {
    return db
      .prepare("SELECT * FROM roles WHERE auth = ?")
      .get(auth) as
      | {
          ip: string;
          auth: string;
  ...
  findByIp(ip: string) {
    return db
      .prepare("SELECT * FROM roles WHERE ip = ?")
      .get(ip) as
      | {
          ip: string;
          auth: string;
  ...
  upsert(ip: string, auth: string, name: string, role: string) {
    db.prepare(`
      INSERT OR REPLACE INTO roles (ip, auth, name, role)
      VALUES (?, ?, ?, ?)
    `).run(ip, auth, name, role);
  },
...
export const bansDb = {
  find(auth: string, ip: string) {
    return db
      .prepare("SELECT * FROM bans WHERE auth = ? OR ip = ?")
      .get(auth, ip) as
      | {
          ip: string;
          auth: string;
  ...
  insert(
    ...
    db.prepare(`
      ...
      (ip, auth, name, banned_by, reason)
      VALUES (?, ?, ?, ?, ?)
    `).run(ip, auth, name, bannedBy ?? "", reason ?? "");
  },

  getAll() {
    return db.prepare("SELECT * FROM bans").all() as {
      ip: string;
      auth: string;
      name: string;
    }[];
...
export const mutesDb = {
  ...
  insert(
    ...
    db.prepare(`
      ...
      (ip, auth, name, muted_by, reason, expires_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(ip, auth, name, mutedBy, reason ?? "", expiresAt);
  },
  ...
  getAllActive() {
    return db
      .prepare("SELECT * FROM mutes WHERE expires_at > unixepoch()")
      .all() as {
      ip: string;
      auth: string;
      name: string;
